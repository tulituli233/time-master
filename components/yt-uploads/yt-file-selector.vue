			...
			<view class="file-select-wrap theme" @click.stop>
				...
				<scroll-view class="s-f-scroll-view theme" scroll-y>
					<view>
						...
						<view @click.stop="showDir(item.isDirectory, item.path)" class="s-f-item"
							v-for="(item, i) in flileList" :key="i">
							<view class="s-f-item-path-name">
							...
							<image :src="source.checked" class="s-f-icon"
								v-if="checkList.indexOf(item.path.toString()) > -1"></image>
						</view>
					</view>
				</scroll-view>
...
export default {
	...
	methods: {
		...
		choose() {
			...
			console.log('this.flileList===', this.flileList);
		},
		backToPrevent() {
			let r = this.flag - 1
			if (r === 0) {
